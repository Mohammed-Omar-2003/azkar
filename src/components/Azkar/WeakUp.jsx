...
import { fetchAzkarWeakUp } from "../../redux/weakUpSlice";
...
function WeakUp() {
  ...
  const { element } = useSelector((state) => state.weakup); //array
  useEffect(() => {
    dispatch(fetchAzkarWeakUp());
  }, [dispatch]);
      ...
      <Container className={style.contaner}>
        {element.map((ele, index) => (
          <div key={ele.id} className={style.box}>
            {/* <h4 style={{ marginBottom: "10px" }}>{ele.pasmala}</h4> */}
            <p>{ele.content}</p>
